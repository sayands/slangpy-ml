...
from network import (
    ...
    LinearLayer, ModuleChain, MultiModuleChain, NoneAct, ReLUAct, SigmoidAct, 
    SwishAct, TanhAct
)
...
def training_main(max_epochs=100, mipmap_level=0):
    ...
    model = MultiModuleChain(*all_models)
    model.initialize(device)
    ...
    target_tex = loader.load_texture("/mnt/sdb/tejan/code/sayan_code/slangpy-ml/inputs/earth.jpg", {"load_as_normalized": True, "generate_mips": True})
    ...
    while app.process_events(): # Change to while True for headless training
        ...
        train = module.trainTextureMulti
...
def inference_main(model_path_base, lod, output_image_path, resolution=512):
    ...
    curr_model.initialize(device)
    
    # Load the saved weights
    prev_model_path = os.path.join(model_path_base, f"model_0.npz")
    load_model_weights(prev_model, prev_model_path)
    
    curr_model_path = os.path.join(model_path_base, f"model_1.npz")
    load_model_weights(curr_model, curr_model_path)
    ...
    module.residualEvalModel(curr_model, prev_model, uv_grid, _result=app.output)
    timer.stop()
...
if __name__ == "__main__":
    ...
    save_path = Path(args.save_dir) / f"model_1.npz"
